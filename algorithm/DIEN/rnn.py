"""
    来自于DIEN的原始实现，文件路径为: https://github.com/mouna99/dien/blob/master/script/rnn.py
    原始实现将TensorFlow的rnn.py中的dynamic_rnn, _dynamic_rnn_loop, _time_step的实现中加入了att_scores参数，并在函数体内增加了相应逻辑
    本实现做了如下改动:
    1. 增加了关于参数att_socre的注释
    2. 函数_like_rnncell在tf1.14中被函数assert_like_rnncell取代，错误直接在assert_like_rnncell中抛出，在调用_like_rnncell的地方都做了相应修改
"""
...
def dynamic_rnn(cell, inputs, att_scores=None, sequence_length=None, initial_state=None,
  ...
  Args:
    ...
    att_scores: attention分数, shape=(B,T,1)
