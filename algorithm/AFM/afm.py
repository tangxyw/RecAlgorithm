...
flags.DEFINE_integer("attention_factor", 128, "Hidden layer size of the attention network")
...
def afm_model_fn(features, labels, mode, params):
    ...
    with tf.variable_scope("pair_interaction_part"):
        # 将每个类别特征的embedding取出
        fields_embeddings = []
        ...
        F = len(params["category_feature_columns"])
        # 存放两两field的哈达玛积
        pair_hadamard_embeddings = []
        for i in range(F):
            for j in range(i+1, F):
                pair_hadamard_embeddings.append(tf.multiply(fields_embeddings[i], fields_embeddings[j]))    # (batch, K) * F*(F-1)/2
        pair_hadamard_embeddings = tf.stack(pair_hadamard_embeddings, axis=1)   # (batch, F*(F-1)/2, K)

    with tf.variable_scope("attention_part"):
        w = tf.get_variable(name="attention_w",
                            shape=(params["embedding_dim"], params["attention_factor"]),
                            dtype=tf.float32)   # (K, t)
        b = tf.get_variable(name="attention_b",
                            shape=(params["attention_factor"]),
                            dtype=tf.float32)   # (t)
        h = tf.get_variable(name="attention_h",
                            shape=(params["attention_factor"], 1),
                            dtype=tf.float32)  # (t, 1)

        # attention_score = h^T * RELU(wX+b)
        attention = tf.matmul(pair_hadamard_embeddings, w) + b  # (batch, F*(F-1)/2, K) matmul (K, t) + (t) = (batch, F*(F-1)/2, t)
        attention = tf.nn.relu(attention)   # (batch, F*(F-1)/2, t)
        attention = tf.matmul(attention, h)  # (batch, F*(F-1)/2, t) matmul (t, 1) = (batch, F*(F-1)/2, 1)
        attention_score = tf.nn.softmax(attention, axis=1)  # (batch, F*(F-1)/2, 1)

    with tf.variable_scope("prediction_score_part"):
        weighted_sum = tf.multiply(pair_hadamard_embeddings, attention_score)   # (batch, F*(F-1)/2, K) * (batch, F*(F-1)/2, 1) = (batch, F*(F-1)/2, K)
        weighted_sum = tf.reduce_sum(weighted_sum, axis=1)  # (batch, K)
        p = tf.get_variable(name="p", shape=(params["embedding_dim"], 1), dtype=tf.float32)  # (K, 1)
        afm_logit = tf.matmul(weighted_sum, p)  # (batch, K) matmul (K, 1) = (batch, 1)
...
def main(unused_argv):
    ...
    params = {
        ...
        "attention_factor": FLAGS.attention_factor,
        "learning_rate": FLAGS.learning_rate,
    }
    print(params)

    estimator = tf.estimator.Estimator(
        model_fn=afm_model_fn,
        params=params,
